...
export function formatEvent(event: Event, view: GameView | null): string {
  ...
  switch (event.type) {
    ...
    case 'revealRequested': return `${playerName(event.revealerIndex)} must reveal a card`;
    case 'revealed': return `${playerName(event.revealerIndex)} revealed a card privately`;
    case 'declinedReveal': return `${playerName(event.revealerIndex)} declined to reveal`;
