...
describe('standalone Discord OAuth', () => {
  ...
  it('exchanges an embedded authorization code for an app session and Discord SDK token', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(jsonResponse({ access_token: ACCESS_TOKEN, token_type: 'Bearer', expires_in: 604_800 }))
      .mockResolvedValueOnce(jsonResponse({ id: '123456789012345678' }));
    const response = await handleOAuth(
  ...
  it.each([
    ['wrong method', new Request('https://example.test/auth/embedded')],
    ['wrong content type', new Request('https://example.test/auth/embedded', {
      method: 'POST',
      body: JSON.stringify({ code: 'code' }),
    })],
    ['invalid JSON', new Request('https://example.test/auth/embedded', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{',
    })],
    ['missing code', new Request('https://example.test/auth/embedded', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{}',
    })],
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch');
    const response = await handleOAuth(request, env());
